...
exports.ExtraAgentsResponse = (() => {
    return v.pipe(v.array(v.object({
        /** Extra agent address. */
        address: v.pipe(_schemas_js_1.Address, v.description("Extra agent address.")),
        /** Extra agent name. */
        name: v.pipe(v.string(), v.nonEmpty(), v.description("Extra agent name.")),
        /** Validity period as a timestamp (in ms since epoch). */
        validUntil: v.pipe(_schemas_js_1.UnsignedInteger, v.description("Validity period as a timestamp (in ms since epoch).")),
...
function extraAgents(config, params, signal) {
    const request = v.parse(exports.ExtraAgentsRequest, {
        type: "extraAgents",
        ...params,
    });
