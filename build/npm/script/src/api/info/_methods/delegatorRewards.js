...
exports.DelegatorRewardsResponse = (() => {
    return v.pipe(v.array(v.object({
        ...
        time: v.pipe(_schemas_js_1.UnsignedInteger, v.description("Timestamp when the reward was received (in ms since epoch).")),
        /** Source of the reward. */
        source: v.pipe(v.picklist(["delegation", "commission"]), v.description("Source of the reward.")),
        /** Total reward amount. */
        totalAmount: v.pipe(_schemas_js_1.UnsignedDecimal, v.description("Total reward amount.")),
...
function delegatorRewards(config, params, signal) {
    const request = v.parse(exports.DelegatorRewardsRequest, {
