...
class WebSocketSubscriptionManager {
    ...
    constructor(socket, postRequest, hlEvents, resubscribe) {
        this._socket = socket;
    ...
    async subscribe(channel, payload, listener) {
        ...
        const id = _postRequest_js_1.WebSocketPostRequest.requestToId(payload);
        ...
        if (!subscription) {
            ...
            if (this._hasUserParam(payload) && this._countUniqueUserSubscriptions() >= MAX_UNIQUE_USER_SUBSCRIPTIONS) {
                throw new _postRequest_js_1.WebSocketRequestError("Cannot track more than 10 unique users");
            }
