...
export const OrderRequest = /* @__PURE__ */ (() => {
    return v.pipe(v.object({
        ...
        action: v.pipe(v.object({
            ...
            orders: v.pipe(v.array(v.object({
                /** Asset ID. */
                ...
                /** Price. */
                p: v.pipe(UnsignedDecimal, v.check((input) => Number(input) > 0, "Value must be greater than zero."), v.description("Price.")),
                ...
                t: v.pipe(v.union([
                    ...
                    v.object({
                        ...
                        trigger: v.pipe(v.object({
                            ...
                            /** Trigger price. */
                            triggerPx: v.pipe(UnsignedDecimal, v.check((input) => Number(input) > 0, "Value must be greater than zero."), v.description("Trigger price.")),
                ...
                /** Client Order ID. */
                c: v.pipe(v.optional(Cloid), v.description("Client Order ID.")),
             ...
             */
            grouping: v.pipe(v.optional(v.picklist(["na", "normalTpsl", "positionTpsl"]), "na"), v.description("Order grouping strategy:" +
                "\n- `na`: Standard order without grouping." +
                "\n- `normalTpsl`: TP/SL order with fixed size that doesn't adjust with position changes." +
                "\n- `positionTpsl`: TP/SL order that adjusts proportionally with the position size.")),
            /** Builder fee. */
            builder: v.pipe(v.optional(v.object({
                ...
                f: v.pipe(UnsignedInteger, v.maxValue(1000), v.description("Builder fee in 0.1bps (1 = 0.0001%). Max 100 for perps (0.1%), 1000 for spot (1%).")),
                ...
                statuses: v.pipe(v.array(v.union([
                    v.object({
                        ...
                        resting: v.pipe(v.object({
                            ...
                            /** Client Order ID. */
                            cloid: v.pipe(v.optional(Cloid), v.description("Client Order ID.")),
