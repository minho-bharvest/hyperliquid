...
import { SpotPairDeployAuctionStatusResponse } from "./spotPairDeployAuctionStatus.js";
/**
    ...
    return v.pipe(v.object({
        ...
        states: v.pipe(v.array(v.object({
            ...
            spec: v.pipe(v.object({
                ...
                weiDecimals: v.pipe(UnsignedInteger, v.description("Number of decimals for the token's smallest unit.")),
            ...
            deployerTradingFeeShare: v.pipe(UnsignedDecimal, v.description("Deployer trading fee share for the token.")),
            /** Spot indices for the token. */
            spots: v.pipe(v.array(UnsignedInteger), v.description("Spot indices for the token.")),
            /** Maximum supply of the token. */
            maxSupply: v.pipe(v.nullable(UnsignedDecimal), v.description("Maximum supply of the token.")),
            /** Hyperliquidity genesis balance of the token. */
            hyperliquidityGenesisBalance: v.pipe(UnsignedDecimal, v.description("Hyperliquidity genesis balance of the token.")),
            /** Total genesis balance (in wei) for the token. */
            totalGenesisBalanceWei: v.pipe(UnsignedDecimal, v.description("Total genesis balance (in wei) for the token.")),
            /** User genesis balances for the token. */
            userGenesisBalances: v.pipe(v.array(v.tuple([Address, UnsignedDecimal])), v.description("User genesis balances for the token.")),
            /** Existing token genesis balances for the token. */
            existingTokenGenesisBalances: v.pipe(v.array(v.tuple([UnsignedInteger, UnsignedDecimal])), v.description("Existing token genesis balances for the token.")),
            /** Blacklisted users for the token. */
            blacklistUsers: v.pipe(v.array(Address), v.description("Blacklisted users for the token.")),
        })), v.description("Array of deploy states for tokens.")),
        /** Status of the spot deploy auction. */
        gasAuction: v.pipe(SpotPairDeployAuctionStatusResponse, v.description("Status of the spot deploy auction.")),
