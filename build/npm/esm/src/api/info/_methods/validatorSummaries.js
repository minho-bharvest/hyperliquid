...
const ValidatorStatsSchema = /* @__PURE__ */ (() => {
    return v.pipe(v.object({
        ...
        predictedApr: v.pipe(UnsignedDecimal, v.description("Predicted annual percentage rate of returns.")),
        /** Number of samples used for statistics calculation. */
        nSamples: v.pipe(UnsignedInteger, v.description("Number of samples used for statistics calculation.")),
...
export const ValidatorSummariesResponse = /* @__PURE__ */ (() => {
    return v.pipe(v.array(v.object({
        ...
        validator: v.pipe(Address, v.description("Address of the validator.")),
        /** Address of the validator signer. */
        signer: v.pipe(Address, v.description("Address of the validator signer.")),
        ...
        nRecentBlocks: v.pipe(UnsignedInteger, v.description("Number of blocks produced recently.")),
        ...
        unjailableAfter: v.pipe(v.nullable(UnsignedInteger), v.description("Timestamp when the validator can be unjailed (in ms since epoch).")),
        ...
        commission: v.pipe(UnsignedDecimal, v.description("Commission rate charged by the validator.")),
 ...
 * ```
 */
export function validatorSummaries(config, paramsOrSignal, maybeSignal) {
