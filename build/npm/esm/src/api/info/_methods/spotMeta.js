...
// API Schemas
// ============================================================
import { Address, Hex, Integer, UnsignedDecimal, UnsignedInteger } from "../../_schemas.js";
/**
...
export const SpotMetaResponse = /* @__PURE__ */ (() => {
    return v.pipe(v.object({
        ...
        universe: v.pipe(v.array(v.object({
            ...
            tokens: v.pipe(v.array(UnsignedInteger), v.description("Token indices included in this universe.")),
            /** Name of the universe. */
            name: v.pipe(v.string(), v.description("Name of the universe.")),
            /** Unique identifier of the universe. */
            index: v.pipe(UnsignedInteger, v.description("Unique identifier of the universe.")),
        ...
        tokens: v.pipe(v.array(v.object({
            ...
            index: v.pipe(UnsignedInteger, v.description("Unique identifier for the token.")),
            /** Token ID. */
            tokenId: v.pipe(Hex, v.description("Token ID.")),
            /** Indicates if the token is the primary representation in the system. */
            isCanonical: v.pipe(v.boolean(), v.description("Indicates if the token is the primary representation in the system.")),
            /** EVM contract details. */
            evmContract: v.pipe(v.nullable(v.object({
                /** Contract address. */
                address: v.pipe(Address, v.description("Contract address.")),
                /** Extra decimals in the token's smallest unit. */
                evm_extra_wei_decimals: v.pipe(Integer, v.description("Extra decimals in the token's smallest unit.")),
...
export function spotMeta(config, paramsOrSignal, maybeSignal) {
