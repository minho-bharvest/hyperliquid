...
export const DelegatorRewardsResponse = /* @__PURE__ */ (() => {
    return v.pipe(v.array(v.object({
        ...
        time: v.pipe(UnsignedInteger, v.description("Timestamp when the reward was received (in ms since epoch).")),
        /** Source of the reward. */
        source: v.pipe(v.picklist(["delegation", "commission"]), v.description("Source of the reward.")),
        /** Total reward amount. */
        totalAmount: v.pipe(UnsignedDecimal, v.description("Total reward amount.")),
